...
<div class="card-header">
    <h3 class="h6 text-uppercase mb-0">Chỉnh sửa Phòng chiếu</h3>
</div>
<div class="card-body">
    <form method="post">
        @csrf
        <div class="form-group row">
            ...
            <div class="col-md-9">
                <input id="inputHorizontalSuccess" value="{{$room->room_name}}" name="room_name" type="text" placeholder="Tên rạp phim" class="form-control form-control-success">
            </div>
        </div>
        <div class="form-group row">
            ...
            <select id="inputHorizontalSuccess" class=" form-control form-control-success" name="cinema_id">
                ...
                <option value="{{$cinema->id}}" {{$cinema->id == $room->cinema_id ? "selected" : ""}}>{{$cinema->cinema_name}}</option>
                @endforeach
            </select>
            </div>
        </div>
